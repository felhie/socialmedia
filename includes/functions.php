...
// Updates   	
	...
	{
    $query = mysql_query("SELECT M.msg_id, M.uid_fk, M.message, M.created, M.tanggal_buat, U.username FROM messages M, users U  WHERE M.uid_fk=U.uid and M.uid_fk='$uid' order by M.msg_id desc ") or die(mysql_error());
	...
	$update=htmlentities($update);
	   ...
	   $ip=$_SERVER['REMOTE_ADDR'];
        ...
        if ($update!=$result['message']) {
            $query = mysql_query("INSERT INTO `messages` (message, uid_fk, ip, created , tanggal_buat) VALUES ('$update', '$uid', '$ip','$time', '$tanggal_buat')") or die(mysql_error());
            $newquery = mysql_query("SELECT M.msg_id, M.uid_fk, M.message, M.created, M.tanggal_buat, U.username FROM messages M, users U where M.uid_fk=U.uid and M.uid_fk='$uid' order by M.msg_id desc limit 1 ");
